...
plot_pbal <- function(pwtest_output,
                      label_option = "minmax",
                      show_color_legend = TRUE){
  pdat <- as.data.frame(pwtest_output$cov_table)
  pdat$`p-value <= .05` <- ifelse(pdat$ttest_p <= 0.05, TRUE, FALSE)

    p <- ggplot(data = pdat) +
      geom_vline(xintercept = 0, linetype = "dashed", alpha = .6) +
      geom_hline(yintercept = 0, linetype = "dashed", alpha = .6) +
      geom_point(aes(x = prognosis, y = dim, color = `p-value <= .05`), show.legend = show_color_legend) +
